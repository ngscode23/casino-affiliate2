...
import { products as localProducts } from "@shared/ecom/data/products";
...
import { getProductBySlug } from "@shared/ecom/api/client";
import type { Product as ProductType } from "@shared/ecom/lib/types";

export default function ProductPage() {
  const { slug = "" } = useParams();
  const localProduct = useMemo(() => localProducts.find((p) => p.slug === slug) || null, [slug]);
  const { add } = useCart();
  const [product, setProduct] = useState<ProductType | null>(localProduct);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!slug) return;
      try {
        // If local dataset has the product, use it; otherwise fetch from API
        if (!localProduct) {
  ...
  // Review integration: only render when product.id looks like a UUID from DB
  const isUuid = (v: string) => /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(v);
  const reviewPk = product && isUuid(product.id) ? product.id : null;

  return (
    <PageShell className="bg-bg text-text">
      <Seo title={product.title} description={product.shortDesc} ogImage={product.imageUrl || product.images[0]} jsonLd={jsonLd} />
      <div className="md:grid md:grid-cols-12 md:gap-6">
        <div className="md:col-span-6">
          <div className="rounded-2xl overflow-hidden border border-border bg-card aspect-[16/10]">
            <img src={product.imageUrl || product.images[0] || "/og.svg"} alt={product.title} className="w-full h-full object-cover" />
          </div>
      ...
      <div className="mt-8 rounded-2xl border border-border bg-card p-4">
        ...
        {reviewPk ? (
          <div className="mt-3">
            <ReviewList sourceSchema="public" sourceTable="ecom_products" sourcePk={reviewPk} reloadKey={reloadKey} />
          </div>
        ) : (
        ...
        {reviewPk ? (
          <div className="mt-4">
            <ReviewForm productId={reviewPk} onSubmitted={() => setReloadKey((k) => k + 1)} />
          </div>
        ) : null}
      </div>
    </PageShell>
