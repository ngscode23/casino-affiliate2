...
function buildSupabaseCreate(params: { supplierId: string; vendorSku: string }) {
  ...
  const createdSku = { id: "sku-1", slug: "vendor-sku-test", sku: "SKU-VENDOR-1", title: "Vendor SKU Test", currency: "USD" };
  const mappingRow = {
    id: "map-1",
  ...
  const from = vi.fn((table: string) => {
    switch (table) {
      ...
      case "ecom_products":
        return { select: ecomProductsSelect, insert: ecomProductsInsert };
      case "supplier_skus":
  ...
  return {
    from,
    supplierSkusInsertMaybeSingle,
    unmappedDeleteEq2,
  };
}

describe("POST /api/admin/supplier-feed/unmapped create_sku", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("creates draft SKU and mapping from unmapped vendor sku", async () => {
    ...
    const { POST } = await import("@/app/api/admin/supplier-feed/unmapped/route");
    const response = await POST(
      new Request("http://localhost/api/admin/supplier-feed/unmapped", {
        method: "POST",
        body: JSON.stringify({
          op: "create_sku",
          supplier_id: supplierId,
