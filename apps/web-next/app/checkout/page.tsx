        ...
        const mapped = await Promise.all(
          items.map(async (row) => {
            ...
            try {
              ...
              // ignore lookup errors; validation will guard later
            }
            return null;
          })
        );
        normalizedItems = mapped.filter((entry): entry is { id: string; qty: number } => Boolean(entry));
      } else {
        normalizedItems = items
          .filter((row) => row.qty > 0)
          .map((row) => ({ id: String(row.id), qty: row.qty }));
      }
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-12 sm:px-6 lg:px-8">
        <div className="space-y-2 text-center sm:text-left">
          ...
          <p className="text-sm text-neutral-600">
            We&apos;ve recorded your payment request and will email updates shortly.
          </p>
        </div>
        ...
        <section className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            ...
            {snapshot.items.map((row) => (
              <li key={row.id} className="flex items-start justify-between gap-3 text-sm text-neutral-600">
                ...
                <span className="text-right font-medium text-slate-900">
                  {formatPrice(row.lineTotal, snapshot.currency)}
                </span>
              </li>
            ))}
          </ul>
          <div className="flex items-center justify-between border-t border-slate-200 pt-3 text-sm text-neutral-600">
            ...
            <span className="font-semibold text-slate-900">
              {formatPrice(snapshot.subtotal, snapshot.currency)}
            </span>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-center gap-3 sm:justify-start">
          <Link
            href="/products"
            className="inline-flex items-center justify-center rounded-lg bg-slate-900 px-5 py-2 text-sm font-medium text-white transition hover:bg-slate-800"
          >
            Continue shopping
          </Link>
          <Link
            href="/account/orders"
            className="inline-flex items-center justify-center rounded-lg border border-slate-200 px-5 py-2 text-sm font-medium text-slate-700 transition hover:bg-slate-50"
          >
            View orders
          </Link>
        </div>
    ...
    return (
      <div className="mx-auto max-w-3xl space-y-6 px-4 py-10 sm:px-6 lg:px-8">
        ...
        <section className="space-y-5 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          {paymentError ? (
            <p className="text-sm text-rose-500">{paymentError}</p>
          ) : null}
          {isStripeProcessing ? (
            <p className="text-sm text-neutral-600">Waiting for confirmation...</p>
          ) : null}
        </section>
