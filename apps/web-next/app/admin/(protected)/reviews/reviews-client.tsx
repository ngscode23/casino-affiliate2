  ...
  return (
    <div className="space-y-6 p-6 text-sm text-white/90">
      <header className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold text-white">Отзывы (одобренные)</h1>
          <p className="text-xs text-white/60">
            Здесь отображаются отзывы со статусом <strong>approved</strong>. Верхние строки — самые свежие.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-xl border border-white/10 bg-white/[0.05] p-4 shadow-lg">
        <dl className="grid gap-4 text-xs sm:grid-cols-3">
          ...
          <div>
            <dt className="text-white/50">Индекс обновления</dt>
            <dd className="text-lg font-semibold text-white">
              {state === "loading" ? "Загрузка…" : new Date().toLocaleTimeString("ru-RU")}
            </dd>
          </div>
          <div>
            <dt className="text-white/50">Доступные действия</dt>
            <dd className="text-white/70">Только просмотр. Модерация выполняется в очереди pending.</dd>
          </div>
        </dl>
      </section>
      ...
      {state !== "error" && (state === "loading" || reviews.length === 0) ? (
      ...
      {state === "ready" && reviews.length > 0 ? (
        <ul className="space-y-4">
          {reviews.map((review) => {
            ...
            return (
              <li
                key={review.review_id ?? `${review.product_uid ?? "unknown"}:${review.reviewer_id ?? "anon"}`}
                className="space-y-3 rounded-2xl border border-white/10 bg-black/30 p-4 shadow-lg"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div className="space-y-1">
                    <div className="text-xs uppercase tracking-[0.24em] text-white/40">Продукт</div>
                    <div className="text-base font-semibold text-white">
                      {review.product_title || "Без названия"}
                      {review.product_slug ? (
                        <Link
                  ...
                  <div className="flex items-center gap-2 text-sm text-white/70">
                    <span className="text-xs uppercase tracking-[0.24em] text-white/40">Оценка</span>
                ...
                <div className="grid gap-4 sm:grid-cols-[minmax(0,1fr)_200px]">
                  <div className="space-y-2">
                    ...
                    <p className="whitespace-pre-line text-sm text-white/80">
                      {sanitize(review.review_body ?? "") || "Текст отзыва отсутствует."}
                    </p>
                  </div>
                  <div className="space-y-2 rounded-xl border border-white/10 bg-white/5 p-3 text-xs text-white/60">
                    <div>
                      <div className="uppercase tracking-[0.24em] text-white/40">ID отзыва</div>
                      <div className="font-mono text-white/70 break-all">{review.review_id ?? "—"}</div>
                    </div>
                    <div>
                      <div className="uppercase tracking-[0.24em] text-white/40">Пользователь</div>
                      <div className="font-mono text-white/70 break-all">{review.reviewer_id ?? "аноним"}</div>
                    </div>
                    <div>
                      <div className="uppercase tracking-[0.24em] text-white/40">Создано</div>
                ...
                {nodes.length ? (
                  <div className="space-y-3 rounded-xl border border-white/10 bg-white/5 p-3">
                    <div className="text-xs uppercase tracking-[0.24em] text-white/40">Переписка</div>
