...
const INVENTORY_FIELDS =
  "id, supplier_id, sku_id, supplier_sku_id, stock_quantity, is_available, inventory_status, last_synced_at, source, metadata, created_at, updated_at";
...
export async function GET(request: Request) {
  ...
  const status = normalizeString(url.searchParams.get("inventory_status"));
  ...
  let query = supabase
    ...
    .select(
      `${INVENTORY_FIELDS}, suppliers(${SUPPLIER_FIELDS}), ecom_products(${SKU_FIELDS}), supplier_skus(${SUPPLIER_SKU_FIELDS})`,
    )
