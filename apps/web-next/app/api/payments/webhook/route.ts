...
function pickSupplierSku(existing: SupplierSkuRow, incoming: SupplierSkuRow): SupplierSkuRow {
  ...
  if (incomingCost == null) return existing;
  return incomingCost < existingCost ? incoming : existing;
}
...
async function createPurchaseOrderForPaidOrder(params: {
  supabase: ReturnType<typeof getAdminClient>;
  orderId: string;
  eventId: string;
  ...
  try {
    ...
    if (supplierError) {
      await recordWebhookLog({
        supabase,
        type: "dropship.po.supplier_lookup_failed",
        status: "error",
        eventId,
        message: supplierError.message || "supplier_skus_lookup_failed",
        payload: { orderId },
      });
      return;
    }

    const supplierBySku = new Map<string, SupplierSkuRow>();
    const supplierIds = new Set<string>();

    for (const row of (supplierRows ?? []) as SupplierSkuRow[]) {
      const skuId = normalizeSkuId(row.sku_id);
      const supplierId = normalizeSkuId(row.supplier_id);
      if (!skuId || !supplierId) continue;
      supplierIds.add(supplierId);
      const existing = supplierBySku.get(skuId);
      supplierBySku.set(skuId, existing ? pickSupplierSku(existing, row) : row);
    }
    ...
    for (const item of resolvedItems) {
      ...
      const payload = {
        ...
        sku_id: item.skuId,
        qty: item.qty,
        cost_cents: typeof skuRow.cost_cents === "number" ? skuRow.cost_cents : null,
        currency: skuRow.currency ?? orderCurrency ?? null,
        supplier_sku_snapshot: skuRow.supplier_sku ?? null,
...
export async function POST(request: Request) {
  ...
  if (event.type === "payment_intent.succeeded") {
    ...
    await createPurchaseOrderForPaidOrder({
      ...
      orderId,
      eventId,
      orderCurrency: currencyForUpdate,
    });
