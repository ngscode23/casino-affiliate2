...
export const handler: Handler = async (event) => {
  try {
    ...
    if (op === "upsert") {
      ...
      const normalizedSku = normalizeSku(product?.sku, product?.title);
      const normalizedSlug = slugifyTitle(product?.slug || product?.title, normalizedSku);
      ...
      const { data, error } = await sb.from("ecom_products").upsert(upsertPayload).select("id").single();
    ...
    if (op === "duplicate") {
      ...
      const rows = (data || []).map((r: any) => {
        const base = slugifyTitle(`${r.slug}-copy-${Math.random().toString(36).slice(2, 6)}`, r.slug);
        return {
          ...r,
          id: undefined,
          slug: base,
          sku: normalizeSku(`${r.sku || r.slug}-copy`, r.slug),
