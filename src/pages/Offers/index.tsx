...
import { getOffersPaged } from "@/features/offers/api/getOffersPaged";
import type { OffersFilterState } from "@/features/offers/components/OfferFiltersFeature";
import { fetchAttributeRegistry, fetchProductAttributes, toValueMap } from "@/lib/attributes";
...
export default function OffersIndex() {
  ...
  return (
    <PageShell>
      ...
      <div className="mb-4 text-sm text-[var(--text-dim)]">{(t("offers.found") || "Found") + ` ${total}`}</div>
      ...
      <div className="md:grid md:grid-cols-12 md:gap-6">
        ...
        <div className="md:col-span-8 lg:col-span-9 space-y-8">
          ...
          {pageLoading && pageItems.length === 0 ? (
            <SectionCard contentClassName="gap-6">
              <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
                {Array.from({ length: 6 }).map((_, i) => (
                  <div key={i} className="rounded-2xl border border-white/10 bg-[var(--bg-1)] p-4 h-[220px] flex flex-col" aria-hidden>
            ...
            >
              ...
              <div className="flex items-center justify-end gap-2">
                <label htmlFor="offers-sort" className="text-sm text-[var(--text-dim)]">{t("offers.sortBy") || "Sort by"}:</label>
                <select
                  id="offers-sort"
                  className="h-10 rounded-xl bg-[rgb(var(--bg-2)/.9)] border border-white/10 px-3 text-sm"
          ...
          {recentOffers.length > 0 && (
            <SectionCard title={t("offers.recent") || "Recently Viewed"} actions={<ButtonGhost onClick={() => { clearRecent(); setRecentVersion((v) => v + 1); }}>{t("common.clear") || "Clear"}</ButtonGhost>}>
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                {recentOffers.map((o) => (
                  <Link key={o.slug} to={`/offers/${encodeURIComponent(o.slug)}`} className="rounded-2xl bg-[color:var(--surface-elev,#141720)] border border-white/5 p-4 hover:opacity-90">
          ...
          <SectionCard>
            <p className="text-neutral-300">{t("offers.tryCompare") || "Want a side-by-side view? Try Compare."}</p>
