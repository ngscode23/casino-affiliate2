  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-white/10">
      <table className="w-full text-sm">
        ...
        <tbody>
          {offers.map((o) => (
            <tr key={o.slug} className="h-16">
              <td className="px-4 py-3 font-medium text-white truncate">
                <span className="truncate inline-block max-w-[90%] align-middle">{o.name}</span>{" "}
                {("pinnedPlan" in (o || {}) || "pinned" in (o || {})) ? <span className="ml-2 align-middle inline-block"><PlanBadge plan={(o as any).pinnedPlan} /></span> : null}
              </td>
              <td className="px-4 py-3"><RatingPillMini value={(o.rating as number) || 0} /></td>
...
export function AffiliateHome() {
  ...
  return (
    <div className="text-white">
      {/* Hide from indexing while casino page is disabled */}
      <Seo title="Casino — temporarily disabled" description="This section is not indexed." noindex />
      {/* Hero */}
      <section className="relative overflow-hidden pt-8">
        <div className="pointer-events-none absolute inset-0 -z-10 opacity-40 [mask-image:radial-gradient(60%_50%_at_50%_0%,#000_10%,transparent_70%)]">
          <div className="absolute left-1/2 top-0 h-[48rem] w-[48rem] -translate-x-1/2 rounded-full bg-[rgb(var(--primary)/.15)] blur-3xl" />
        </div>
        <PageShell>
          <div className="grid items-start gap-8 md:grid-cols-2">
            <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
              ...
              <div className="mt-6 grid max-w-lg grid-cols-3 gap-3 text-sm text-white/80">
                ...
                <div className="rounded-xl bg-white/5 p-3 border border-white/10">Проверена скорость выплат</div>
                <div className="rounded-xl bg-white/5 p-3 border border-white/10">Без ловушек KYC</div>
              </div>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.1 }}>
              <SectionCard title="Выбор дня">
                {error ? (
      ...
      <section className="py-12 md:py-16">
        <PageShell>
          <div className="grid grid-cols-1 gap-6 md:grid-cols-4">
            ...
            <SectionCard>
              <div className="flex items-start gap-3">
                <Timer className="h-5 w-5 text-[rgb(var(--primary))]" />
                <div>
            ...
            <SectionCard>
              <div className="flex items-start gap-3">
                <TrendingUp className="h-5 w-5 text-[rgb(var(--primary))]" />
                <div>
