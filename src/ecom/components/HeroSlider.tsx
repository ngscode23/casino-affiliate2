...
export function HeroSlider() {
  ...
  return (
    <section
      className="relative overflow-hidden rounded-none md:rounded-2xl border border-white/10"
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
      aria-roledescription="carousel"
    >
      {/* Viewport */}
      <div className="relative h-[34vw] min-h-[220px] max-h-[520px] bg-black">
        ...
        {/* Overlays */}
        <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_bottom,rgba(0,0,0,.25),rgba(0,0,0,.05)_40%,rgba(0,0,0,.35))]" aria-hidden />
        ...
        <div className="relative z-[1] px-4 sm:px-6 lg:px-8 h-full flex items-center">
          <div className="max-w-[720px]">
            ...
            {current.subtitle ? (
              <p className="mt-3 text-white/90 text-lg">{current.subtitle}</p>
            ) : null}
            {current.cta ? (
              <Link
                to={current.cta.href}
                className="mt-5 inline-flex items-center justify-center rounded-full bg-yellow-400 text-black font-semibold px-5 py-2 hover:bg-yellow-300"
              >
                {current.cta.label}
              </Link>
            ) : null}
