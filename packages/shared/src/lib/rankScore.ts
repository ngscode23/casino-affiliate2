import type { NormalizedOffer } from "./offers";

const CURACAO_REGEX = /curacao|cura(?:c|\u00E7)ao/i;
...
function isMediumLicense(lic?: string) {
  if (!lic) return false;
  return CURACAO_REGEX.test(lic.trim());
}

export function rankScore(
  o: NormalizedOffer | { rating?: number; payoutHours?: number; license?: string }
): number {
