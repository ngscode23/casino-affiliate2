...
export const SUPABASE_PUBLISHABLE_KEY: string = ((import.meta as any).env?.VITE_SUPABASE_PUBLISHABLE_KEY || "").trim();
const RAW_SUPABASE_PRODUCT_BUCKET: string = ((import.meta as any).env?.VITE_SUPABASE_PRODUCT_BUCKET ?? "").trim();
...
export const HAS_SUPABASE: boolean =
  /^https?:\/\/.+\.supabase\.co$/i.test(SUPABASE_URL) && !!SUPABASE_PUBLISHABLE_KEY;
